  ...
  return Object.fromEntries(
    (req.headers.cookie || '')
      ...
      .map((part) => {
        ...
        if (i < 0) return ['', ''];
        return [decodeURIComponent(part.slice(0, i).trim()), decodeURIComponent(part.slice(i + 1))];
      })
      .filter(([key]) => key)
  );
}
...
function setSession(res, admin) {
  ...
  res.setHeader('Set-Cookie', `${COOKIE_NAME}=${encodeURIComponent(token)}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${SESSION_HOURS * 3600}${secure}`);
}
...
function setTeacherSession(res, teacher) {
  ...
  const secure = process.env.NODE_ENV === 'production' ? '; Secure' : '';
  res.setHeader('Set-Cookie', `${TEACHER_COOKIE_NAME}=${encodeURIComponent(token)}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${SESSION_HOURS * 3600}${secure}`);
}

function clearTeacherSession(res) {
  res.setHeader('Set-Cookie', `${TEACHER_COOKIE_NAME}=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0`);
}

function teacherAuth(req, res, next) {
  try {
    const payload = verify(parseCookies(req)[TEACHER_COOKIE_NAME]);
    if (!payload || payload.role !== 'teacher') return res.status(401).json({ error: 'Unauthorized. Please log in.' });
    req.teacher = payload;
    next();
  } catch {
...
async function ensureStructure() {
  ...
  if (!cols.length) {
    await pool.query(`ALTER TABLE results ADD COLUMN status VARCHAR(20) NOT NULL DEFAULT 'DRAFT' AFTER remark`);
    await pool.query(`UPDATE results SET status = CASE WHEN published=1 THEN 'PUBLISHED' ELSE 'DRAFT' END`);
  }
}
...
app.get('/api/public/options', async (_req, res) => {
  try {
    const [sessions] = await pool.query('SELECT id,name FROM academic_sessions WHERE active=1 ORDER BY id DESC');
...
app.post('/api/result/check', async (req, res) => {
  try {
    ...
    if (!admission || !pin || !sessionId || !termId) {
      return res.status(400).json({ error: 'Admission number, PIN, session and term are required.' });
    }
    ...
    if (!token) {
      return res.status(404).json({ error: 'No result found. Check the admission number, PIN, session and term.' });
    }
    ...
    if (!rows.length) return res.status(404).json({ error: 'This result has not been published yet.' });

    const [[session]] = await pool.query('SELECT name FROM academic_sessions WHERE id=?', [sessionId]);
    const [[term]] = await pool.query('SELECT name FROM terms WHERE id=?', [termId]);
    const total = rows.reduce((sum, row) => sum + Number(row.total_score || 0), 0);
    const average = rows.length ? total / rows.length : 0;

    await pool.query('UPDATE result_pins SET use_count=use_count+1, last_used_at=NOW() WHERE id=?', [token.id]);

    res.json({
      student: {
        admission_number: token.admission_number,
        name: [token.first_name, token.other_name, token.last_name].filter(Boolean).join(' '),
    ...
    res.status(500).json({ error: 'Unable to check result.', detail: error.message });
  }
});
...
app.post('/api/admin/login', async (req, res) => {
  try {
    ...
    const [[admin]] = await pool.query('SELECT * FROM admins WHERE username=? AND active=1 LIMIT 1', [username]);
    if (!admin || !verifyPassword(password, admin.password_hash)) {
      return res.status(401).json({ error: 'Invalid username or password.' });
    }

    setSession(res, admin);
    res.json({ ok: true, admin: { id: admin.id, username: admin.username, name: admin.full_name } });
  } catch (error) {
...
app.get('/api/admin/dashboard', auth, async (_req, res) => {
  try {
    const [studentRows, classRows, subjectRows, resultRows, publishedRows, pinRows] = await Promise.all([
...
app.put('/api/admin/students/:id', auth, async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      ...
      [admission, first, last, other || null, gender || null, classId, active, id]
    );
...
app.patch('/api/admin/students/:id/status', auth, async (req, res) => {
  try {
    ...
    const [result] = await pool.query('UPDATE students SET active=? WHERE id=?', [active, id]);
    if (!result.affectedRows) return res.status(404).json({ error: 'Student not found.' });
...
app.put('/api/admin/classes/:id', auth, async (req, res) => {
  try {
    const id = Number(req.params.id || 0);
    const name = String(req.body.name || '').trim();
    if (!id || !name) return res.status(400).json({ error: 'Valid class ID and class name are required.' });
    const [result] = await pool.query('UPDATE classes SET name=? WHERE id=?', [name, id]);
...
app.delete('/api/admin/classes/:id', auth, async (req, res) => {
  try {
    const id = Number(req.params.id || 0);
    const [[students]] = await pool.query('SELECT COUNT(*) AS n FROM students WHERE class_id=?', [id]);
    if (Number(students.n) > 0) {
      return res.status(409).json({ error: 'This class cannot be deleted because students are assigned to it.' });
    }
...
app.post('/api/admin/subjects', auth, async (req, res) => {
  try {
    ...
    const [result] = await pool.query('INSERT INTO subjects (name,code) VALUES (?,?)', [name, code || null]);
    res.status(201).json({ ok: true, id: result.insertId });
  } catch (error) {
    res.status(error.code === 'ER_DUP_ENTRY' ? 409 : 500).json({
      error: duplicateMessage(error, 'A subject with this name already exists.')
    });
  }
});
...
app.put('/api/admin/subjects/:id', auth, async (req, res) => {
  try {
    ...
    const name = String(req.body.name || '').trim();
    const code = String(req.body.code || '').trim().toUpperCase();
    if (!id || !name) return res.status(400).json({ error: 'Valid subject ID and subject name are required.' });
    const [result] = await pool.query('UPDATE subjects SET name=?,code=? WHERE id=?', [name, code || null, id]);
...
app.delete('/api/admin/subjects/:id', auth, async (req, res) => {
  try {
    const id = Number(req.params.id || 0);
    const [[results]] = await pool.query('SELECT COUNT(*) AS n FROM results WHERE subject_id=?', [id]);
    if (Number(results.n) > 0) {
      return res.status(409).json({ error: 'This subject cannot be deleted because result records already use it.' });
    }
...
app.put('/api/admin/class-subjects/:classId', auth, async (req, res) => {
  ...
  try {
    ...
    if (subjectIds.length) {
      await conn.query(`INSERT INTO class_subjects (class_id,subject_id) VALUES ${subjectIds.map(() => '(?,?)').join(',')}`, subjectIds.flatMap(id => [classId,id]));
    }
    await conn.commit();
...
app.post('/api/admin/teachers', auth, async (req, res) => {
  try {
    ...
    if (!username || !fullName || password.length < 8) return res.status(400).json({ error: 'Username, full name and a password of at least 8 characters are required.' });
    const [result] = await pool.query('INSERT INTO teachers (username,full_name,email,phone,password_hash,active) VALUES (?,?,?,?,?,1)', [username,fullName,email||null,phone||null,hashPassword(password)]);
    res.status(201).json({ ok:true,id:result.insertId });
  } catch (error) { res.status(error.code==='ER_DUP_ENTRY'?409:500).json({ error: duplicateMessage(error,'A teacher with this username already exists.') }); }
});

app.put('/api/admin/teachers/:id', auth, async (req, res) => {
  try {
    const id=Number(req.params.id||0), username=String(req.body.username||'').trim(), fullName=String(req.body.full_name||'').trim();
    const email=String(req.body.email||'').trim(), phone=String(req.body.phone||'').trim(), active=req.body.active===0||req.body.active===false||req.body.active==='0'?0:1;
    if(!id||!username||!fullName) return res.status(400).json({error:'Teacher ID, username and full name are required.'});
    const password=String(req.body.password||'');
    if(password && password.length<8) return res.status(400).json({error:'New teacher password must be at least 8 characters.'});
    let sql='UPDATE teachers SET username=?,full_name=?,email=?,phone=?,active=?'; const params=[username,fullName,email||null,phone||null,active];
    if(password){ sql+=',password_hash=?'; params.push(hashPassword(password)); } sql+=' WHERE id=?'; params.push(id);
    const [result]=await pool.query(sql,params); if(!result.affectedRows) return res.status(404).json({error:'Teacher not found.'}); res.json({ok:true});
  } catch(error){res.status(error.code==='ER_DUP_ENTRY'?409:500).json({error:duplicateMessage(error,'Another teacher already uses this username.')});}
});

app.delete('/api/admin/teachers/:id', auth, async (req,res)=>{
  try { const id=Number(req.params.id||0); const [result]=await pool.query('UPDATE teachers SET active=0 WHERE id=?',[id]); if(!result.affectedRows)return res.status(404).json({error:'Teacher not found.'}); res.json({ok:true}); }
  catch(error){res.status(500).json({error:error.message});}
});

app.get('/api/admin/teacher-assignments', auth, async (_req,res)=>{
  try { const [rows]=await pool.query(`SELECT ta.id,ta.teacher_id,ta.subject_id,ta.class_id,ta.active,t.full_name teacher_name,t.username,s.name subject_name,c.name class_name FROM teacher_assignments ta JOIN teachers t ON t.id=ta.teacher_id JOIN subjects s ON s.id=ta.subject_id JOIN classes c ON c.id=ta.class_id ORDER BY t.full_name,c.name,s.name`); res.json(rows); }
  catch(error){res.status(500).json({error:error.message});}
});

app.post('/api/admin/teacher-assignments', auth, async (req,res)=>{
  try { const teacherId=Number(req.body.teacher_id||0),subjectId=Number(req.body.subject_id||0),classId=Number(req.body.class_id||0); if(!teacherId||!subjectId||!classId)return res.status(400).json({error:'Teacher, subject and class are required.'}); const [[offered]]=await pool.query('SELECT id FROM class_subjects WHERE class_id=? AND subject_id=?',[classId,subjectId]); if(!offered)return res.status(400).json({error:'Add this subject to the class subject arrangement before assigning a teacher.'}); const [r]=await pool.query('INSERT INTO teacher_assignments (teacher_id,subject_id,class_id,active) VALUES (?,?,?,1)',[teacherId,subjectId,classId]); res.status(201).json({ok:true,id:r.insertId}); }
  catch(error){res.status(error.code==='ER_DUP_ENTRY'?409:500).json({error:duplicateMessage(error,'This teacher is already assigned to that subject and class.')});}
});
app.delete('/api/admin/teacher-assignments/:id', auth, async (req,res)=>{ try{const [r]=await pool.query('DELETE FROM teacher_assignments WHERE id=?',[Number(req.params.id)]); if(!r.affectedRows)return res.status(404).json({error:'Assignment not found.'});res.json({ok:true});}catch(e){res.status(500).json({error:e.message});} });

// ------------------------------------------------------------
// Teacher portal
// ------------------------------------------------------------
app.post('/api/teacher/login', async (req,res)=>{
  try { const username=String(req.body.username||'').trim(),password=String(req.body.password||''); const [[teacher]]=await pool.query('SELECT * FROM teachers WHERE username=? AND active=1 LIMIT 1',[username]); if(!teacher||!verifyPassword(password,teacher.password_hash))return res.status(401).json({error:'Invalid username or password.'}); setTeacherSession(res,teacher); res.json({ok:true,teacher:{id:teacher.id,username:teacher.username,name:teacher.full_name}}); }
  catch(error){res.status(500).json({error:error.message});}
});
app.post('/api/teacher/logout',(req,res)=>{clearTeacherSession(res);res.json({ok:true});});
app.get('/api/teacher/me',teacherAuth,(req,res)=>res.json({teacher:req.teacher}));
app.get('/api/teacher/dashboard',teacherAuth,async(req,res)=>{try{const [assignments]=await pool.query(`SELECT ta.id,ta.subject_id,ta.class_id,s.name subject_name,s.code subject_code,c.name class_name FROM teacher_assignments ta JOIN subjects s ON s.id=ta.subject_id JOIN classes c ON c.id=ta.class_id JOIN class_subjects cs ON cs.class_id=ta.class_id AND cs.subject_id=ta.subject_id WHERE ta.teacher_id=? AND ta.active=1 ORDER BY s.name,c.name`,[req.teacher.id]);res.json({teacher:req.teacher,assignments});}catch(e){res.status(500).json({error:e.message});}});
app.get('/api/teacher/students',teacherAuth,async(req,res)=>{try{const assignmentId=Number(req.query.assignment_id||0);const sessionId=Number(req.query.session_id||0);const termId=Number(req.query.term_id||0);const [[a]]=await pool.query('SELECT ta.*,s.name subject_name,c.name class_name FROM teacher_assignments ta JOIN subjects s ON s.id=ta.subject_id JOIN classes c ON c.id=ta.class_id JOIN class_subjects cs ON cs.class_id=ta.class_id AND cs.subject_id=ta.subject_id WHERE ta.id=? AND ta.teacher_id=? AND ta.active=1',[assignmentId,req.teacher.id]);if(!a)return res.status(403).json({error:'You are not assigned to this subject and class.'});if(!sessionId||!termId)return res.status(400).json({error:'Session and term are required.'});const [students]=await pool.query(`SELECT st.id,st.admission_number,st.first_name,st.last_name,st.other_name,r.ca_score,r.exam_score,r.total_score,r.grade,r.remark,r.status,r.published FROM students st LEFT JOIN results r ON r.student_id=st.id AND r.subject_id=? AND r.session_id=? AND r.term_id=? WHERE st.class_id=? AND st.active=1 ORDER BY st.last_name,st.first_name`,[a.subject_id,sessionId,termId,a.class_id]);res.json({assignment:a,students});}catch(e){res.status(500).json({error:e.message});}});
app.get('/api/teacher/meta',teacherAuth,async(_req,res)=>{try{const [sessions]=await pool.query('SELECT id,name,active FROM academic_sessions ORDER BY id DESC');const [terms]=await pool.query('SELECT id,name FROM terms ORDER BY id');res.json({sessions,terms});}catch(e){res.status(500).json({error:e.message});}});
app.post('/api/teacher/results',teacherAuth,async(req,res)=>{const conn=await pool.getConnection();try{const assignmentId=Number(req.body.assignment_id||0),sessionId=Number(req.body.session_id||0),termId=Number(req.body.term_id||0),items=Array.isArray(req.body.results)?req.body.results:[];const [[a]]=await conn.query('SELECT ta.*,cs.id class_subject_id FROM teacher_assignments ta JOIN class_subjects cs ON cs.class_id=ta.class_id AND cs.subject_id=ta.subject_id WHERE ta.id=? AND ta.teacher_id=? AND ta.active=1',[assignmentId,req.teacher.id]);if(!a)return res.status(403).json({error:'You are not assigned to this subject and class.'});if(!sessionId||!termId||!items.length)return res.status(400).json({error:'Assignment, session, term and scores are required.'});await conn.beginTransaction();for(const item of items){const studentId=Number(item.student_id||0),ca=Number(item.ca_score||0),exam=Number(item.exam_score||0);const [[student]]=await conn.query('SELECT id FROM students WHERE id=? AND class_id=? AND active=1',[studentId,a.class_id]);if(!student)throw Object.assign(new Error('A student does not belong to this class.'),{statusCode:400});if(ca<0||ca>40||exam<0||exam>60)throw Object.assign(new Error('CA must be 0-40 and Exam must be 0-60.'),{statusCode:400});const total=Number((ca+exam).toFixed(2));const[g,remark]=grade(total);await conn.query(`INSERT INTO results (student_id,subject_id,session_id,term_id,ca_score,exam_score,total_score,grade,remark,status,published) VALUES (?,?,?,?,?,?,?,?,?,'DRAFT',0) ON DUPLICATE KEY UPDATE ca_score=VALUES(ca_score),exam_score=VALUES(exam_score),total_score=VALUES(total_score),grade=VALUES(grade),remark=VALUES(remark),status=IF(results.status='PUBLISHED','PUBLISHED','DRAFT'),published=IF(results.published=1,1,0)`,[studentId,a.subject_id,sessionId,termId,ca,exam,total,g,remark]);}await conn.commit();res.json({ok:true});}catch(e){await conn.rollback();res.status(e.statusCode||500).json({error:e.message});}finally{conn.release();}});
app.post('/api/teacher/results/submit',teacherAuth,async(req,res)=>{try{const assignmentId=Number(req.body.assignment_id||0),sessionId=Number(req.body.session_id||0),termId=Number(req.body.term_id||0);const [[a]]=await pool.query('SELECT * FROM teacher_assignments WHERE id=? AND teacher_id=? AND active=1',[assignmentId,req.teacher.id]);if(!a)return res.status(403).json({error:'You are not assigned to this subject and class.'});const [r]=await pool.query("UPDATE results SET status='SUBMITTED' WHERE subject_id=? AND session_id=? AND term_id=? AND student_id IN (SELECT id FROM students WHERE class_id=? AND active=1) AND status<>'PUBLISHED'",[a.subject_id,sessionId,termId,a.class_id]);res.json({ok:true,updated:r.affectedRows});}catch(e){res.status(500).json({error:e.message});}});
...
app.get('/api/admin/sessions', auth, async (_req, res) => {
  try {
    const [rows] = await pool.query('SELECT id,name,active FROM academic_sessions ORDER BY id DESC');
    res.json(rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.post('/api/admin/sessions', auth, async (req, res) => {
  try {
    const name = String(req.body.name || '').trim();
    if (!/^\d{4}\/\d{4}$/.test(name)) return res.status(400).json({ error: 'Use session format YYYY/YYYY, for example 2026/2027.' });
    const [result] = await pool.query('INSERT INTO academic_sessions (name,active) VALUES (?,1)', [name]);
    res.status(201).json({ ok: true, id: result.insertId });
  } catch (error) {
    res.status(error.code === 'ER_DUP_ENTRY' ? 409 : 500).json({
...
app.patch('/api/admin/sessions/:id/status', auth, async (req, res) => {
  try {
    const id = Number(req.params.id || 0);
    const active = req.body.active === true || req.body.active === 1 || req.body.active === '1' ? 1 : 0;
    const [result] = await pool.query('UPDATE academic_sessions SET active=? WHERE id=?', [active, id]);
    if (!result.affectedRows) return res.status(404).json({ error: 'Academic session not found.' });
    res.json({ ok: true, active: Boolean(active) });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/api/admin/broadsheet', auth, async (req, res) => {
  try {
    ...
    if (!classId || !sessionId || !termId) return res.status(400).json({ error: 'Class, session and term are required.' });

    const [[cls]] = await pool.query('SELECT id,name FROM classes WHERE id=?', [classId]);
    if (!cls) return res.status(404).json({ error: 'Class not found.' });
    const [subjects] = await pool.query(`
      SELECT s.id,s.name,s.code
      FROM class_subjects cs JOIN subjects s ON s.id=cs.subject_id
      WHERE cs.class_id=? ORDER BY s.name`, [classId]);
    const [students] = await pool.query(`
      SELECT st.id,st.admission_number,st.first_name,st.last_name,st.other_name
      FROM students st WHERE st.class_id=? AND st.active=1
      ORDER BY st.last_name,st.first_name,st.other_name`, [classId]);
    const [results] = await pool.query(`
      ...
      WHERE st.class_id=? AND st.active=1 AND r.session_id=? AND r.term_id=?`, [classId,sessionId,termId]);
    const map = {};
...
app.get('/api/admin/submission-tracking', auth, async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(`
      ...
      ORDER BY c.name,t.full_name,s.name`, [sessionId,termId]);
    const data = rows.map(r => ({
      ...r,
      status: Number(r.published_count) > 0 && Number(r.published_count) >= Number(r.student_count) ? 'PUBLISHED' :
        Number(r.submitted_count) > 0 && Number(r.submitted_count) >= Number(r.student_count) ? 'SUBMITTED' :
...
app.post('/api/admin/results', auth, async (req, res) => {
  ...
  try {
    ...
    if (!studentId || !sessionId || !termId || !Array.isArray(results)) {
      return res.status(400).json({ error: 'Student, session, term and results are required.' });
    }

    const [[student]] = await conn.query('SELECT class_id FROM students WHERE id=? AND active=1', [studentId]);
    ...
    for (const item of results) {
      ...
      if (!Number.isFinite(ca) || ca < 0 || ca > 40) throw new Error('CA scores must be between 0 and 40.');
      if (!Number.isFinite(exam) || exam < 0 || exam > 60) throw new Error('Exam scores must be between 0 and 60.');
      const [[offered]] = await conn.query('SELECT id FROM class_subjects WHERE class_id=? AND subject_id=?', [student.class_id, subjectId]);
      if (!offered) throw new Error('This subject is not offered by the student’s class.');
      const total = ca + exam;
      ...
      await conn.query(
        ...
        [studentId, subjectId, sessionId, termId, ca, exam, total, letter, remark]
      );
    }
    await conn.commit();
    res.json({ ok: true });
...
app.post('/api/admin/results/publish', auth, async (req, res) => {
  try {
    ...
    const sessionId = Number(req.body.session_id || 0);
    const termId = Number(req.body.term_id || 0);
    const published = req.body.published === true || req.body.published === 1 || req.body.published === '1';
    if (!studentId || !sessionId || !termId) return res.status(400).json({ error: 'Student, session and term are required.' });

    const [result] = await pool.query(
      'UPDATE results SET published=?, status=? WHERE student_id=? AND session_id=? AND term_id=?',
      [published ? 1 : 0, published ? 'PUBLISHED' : 'DRAFT', studentId, sessionId, termId]
    );
...
app.patch('/api/admin/pins/:id/status', auth, async (req, res) => {
  try {
    const id = Number(req.params.id || 0);
    const active = req.body.active === true || req.body.active === 1 || req.body.active === '1' ? 1 : 0;
    const [result] = await pool.query('UPDATE result_pins SET active=? WHERE id=?', [active, id]);
    if (!result.affectedRows) return res.status(404).json({ error: 'PIN not found.' });
    res.json({ ok: true, active: Boolean(active) });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.delete('/api/admin/pins/:id', auth, async (req, res) => {
  try {
    const id = Number(req.params.id || 0);
    const [result] = await pool.query('DELETE FROM result_pins WHERE id=?', [id]);
    if (!result.affectedRows) return res.status(404).json({ error: 'PIN not found.' });
    res.json({ ok: true });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.post('/api/admin/change-password', auth, async (req, res) => {
  try {
    ...
    const [[admin]] = await pool.query('SELECT id,password_hash FROM admins WHERE id=? LIMIT 1', [req.admin.id]);
    if (!admin || !verifyPassword(current, admin.password_hash)) return res.status(400).json({ error: 'Current password is incorrect.' });
    await pool.query('UPDATE admins SET password_hash=? WHERE id=?', [hashPassword(next), req.admin.id]);
    res.json({ ok: true });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
async function startServer() {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log('Class Subject API loaded: /api/admin/class-subjects/:classId');
    });
  } catch (error) {
